...
def write_graph(graph, start, dist, stack=None):
    ...
    for i in range(len(graph)):
        # blue edges for all shortest paths starting from 'start'
        digraph.add_edge(pydot.Edge(start, i, label=dist[i], fontcolor='blue', color='blue'))
    if stack:
        s = stack[0]
        c = 0
        for n in stack[1:]:
            # red edges for the shortest paths from 'start' to 'dst'
            c += graph[s][n]
